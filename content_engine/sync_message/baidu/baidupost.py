...
class BaiduUser(object):
    ...
    @staticmethod
    def text_wrapped_by(self,start, end, content):
        """get the text wrapped by start、end in content"""
        if content is None:
            return None
        si = content.find(start)
    ...
    def login(self, verify_code=""):
        ...
        print '123' + content
        body = self.text_wrapped_by('<body onload="sett_pwd_load()">', '</body>', content)
        err_code = self.text_wrapped_by("get_err_str(", ",", body)
        # 如果是需要输入验证码
        if err_code == '257':
            print "需要输入验证码，重新登录中..."
